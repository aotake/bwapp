...
class Form_Validator_Role_Edit extends Form_Validator_Role_New
{
    ...
    public function isRegistedRid()
    {
        ...
        $post_val = $req->getPost("rid");

        $model = new Model_Role();
        $select = $model->select();
        $select->where("rid = ?", $req->getParam("rid_orig"));
