...
$zendlib_top = realpath($webapp_dir."/../libs");
include_once $webapp_dir."/bootstrap.php";
$conf = Webapp::loadConfig($webapp_dir, null, $zendlib_top);
//
 ...
 *
 */
define('OWA_DB_TYPE', $conf->db->type); // options: mysql
define('OWA_DB_NAME', $conf->db->dsn->dbname); // name of the database
define('OWA_DB_HOST', $conf->db->dsn->host); // host name of the server housing the database
define('OWA_DB_USER', $conf->db->dsn->username); // database user
define('OWA_DB_PASSWORD', $conf->db->dsn->password); // database user's password
...
define('OWA_PUBLIC_URL', $conf->site->root_url.'/owa/');
