...
if [ -d ../modules/$TO_MODNAME/libs/custom/${FROM_PASCAL}Custom ]; then
    echo "mv ../modules/$TO_MODNAME/libs/custom/${FROM_PASCAL}Custom ../modules/$TO_MODNAME/libs/custom/${TO_PASCAL}Custom"
    mv ../modules/$TO_MODNAME/libs/custom/${FROM_PASCAL}Custom ../modules/$TO_MODNAME/libs/custom/${TO_PASCAL}Custom
fi
# 古い libs は削除
...
CONF_DEFAULT=../modules/$TO_MODNAME/config/config.ini
CONF_CUSTOM=../modules/$TO_MODNAME/config/custom/config.ini
if [ -f $CONF_DEFAULT ]; then
    echo "convert default config.ini...."
    cat $CONF_DEFAULT | sed "s/^${FROM_MODNAME}/${TO_MODNAME}/" > tmp
    echo "----> mv tmp $CONF_DEFAULT"
    mv tmp  $CONF_DEFAULT
fi
if [ -f $CONF_CUSTOM ]; then
    echo "convert custom config.ini...."
    cat $CONF_CUSTOM | sed "s/^${FROM_MODNAME}/${TO_MODNAME}/" > tmp
    echo "----> mv tmp $CONF_CUSTOM"
    mv tmp  $CONF_CUSTOM
fi
